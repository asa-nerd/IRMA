...
public float getAllActivity(int time){
  //for(int t = 0; t < 100; t ++){                         // loop through whole piece (datapoints)
    float globalActivity = 0;
    
    for(int i = 0; i < subjects.size(); i++){            // loop thorugh all subjects
        Subject s = subjects.get(i);                
        Point currentPoint = s.PointsList.get(time);      
        float pointActivity = currentPoint.activity;     // get activity of each subject at current time
        globalActivity += pointActivity;                 // add to global activity at this time
        //println("hi");
    }
